...
let words = message.split(" ").map(word => word.toLowerCase());
let onlyNewWords = Array.from(new Set(words));

let finalMessage = '';
let count = 0;

for (let i = 0; i < onlyNewWords.length; i++) {
    count = 0;
    for (let j = 0; j < words.length; j++) {
        if (onlyNewWords[i] === words[j]) {
            count++;
        }
    }
    finalMessage += `${onlyNewWords[i]}${count}`
}
